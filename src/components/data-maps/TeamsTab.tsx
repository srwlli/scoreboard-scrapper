...
function getTotalFields(): number {
  return TEAMS_SECTIONS.reduce((sum, section) => sum + section.fields.length, 0)
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Teams Page Fields</CardTitle>
            <Badge variant="outline">{totalFields} fields</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Query: <code className="bg-muted px-1 rounded">teams-queries.ts</code> → <code className="bg-muted px-1 rounded">getTeamsByDivision()</code>, <code className="bg-muted px-1 rounded">getTeamTransactions()</code>
          </p>
        </CardHeader>
      </Card>

      {TEAMS_SECTIONS.map((section) => (
