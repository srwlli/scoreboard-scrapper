...
function getTotalFields(): number {
  return GAME_DETAILS_SECTIONS.reduce((sum, section) => sum + section.fields.length, 0)
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Game Details Page Fields</CardTitle>
            <Badge variant="outline">{totalFields} fields</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Query: <code className="bg-muted px-1 rounded">game-queries.ts</code> → <code className="bg-muted px-1 rounded">getGameDetails()</code>
          </p>
          <p className="text-sm text-muted-foreground">
            {GAME_DETAILS_SECTIONS.length} sections with parallel Supabase queries
          </p>
        </CardHeader>
      </Card>

      {GAME_DETAILS_SECTIONS.map((section) => (
