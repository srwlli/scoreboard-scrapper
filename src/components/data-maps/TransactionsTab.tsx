...
function getTotalFields(): number {
  return TRANSACTIONS_SECTIONS.reduce((sum, section) => sum + section.fields.length, 0)
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Transactions Page Fields</CardTitle>
            <Badge variant="outline">{totalFields} fields</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Query: <code className="bg-muted px-1 rounded">transactions-queries.ts</code> → <code className="bg-muted px-1 rounded">getTeamTransactions()</code>
          </p>
        </CardHeader>
      </Card>

      {TRANSACTIONS_SECTIONS.map((section) => (
