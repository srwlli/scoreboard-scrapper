...
function getTotalFields(): number {
  return SCOREBOARD_SECTIONS.reduce((sum, section) => sum + section.fields.length, 0)
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Scoreboard Page Fields</CardTitle>
            <Badge variant="outline">{totalFields} fields</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Query: <code className="bg-muted px-1 rounded">scoreboard-queries.ts</code> → <code className="bg-muted px-1 rounded">getGamesForWeek()</code>
          </p>
        </CardHeader>
      </Card>

      {SCOREBOARD_SECTIONS.map((section) => (
