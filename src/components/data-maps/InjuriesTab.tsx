...
function getTotalFields(): number {
  return INJURIES_SECTIONS.reduce((sum, section) => sum + section.fields.length, 0)
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Injuries Page Fields</CardTitle>
            <Badge variant="outline">{totalFields} fields</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Query: <code className="bg-muted px-1 rounded">injuries-queries.ts</code> → <code className="bg-muted px-1 rounded">getAllInjuries()</code>
          </p>
        </CardHeader>
      </Card>

      {INJURIES_SECTIONS.map((section) => (
